...
void LidDrivenCavityExp::UpdateTemps(int* ind){
 
    for (int i=ind[0];i<ind[1];i++){
            for (int j=ind[2];j<ind[3];j++){
                if (i==0){
                    stemp[i*Nyloc+j]=y_right[j]; 
                   // vtemp[i*Nyloc+j]=x_right[j];
                }
                else{
                    stemp[i*Nyloc+j]=s[(i-1)*Nyloc+j]; 
                   // vtemp[i*Nyloc+j]=v[(i-1)*Nyloc+j]; 
                }
            }
    }
}
//                if (i==Nxloc-1){
//                    stemp1[i*Nyloc+j]=y_left[j]; 
//                     vtemp1[i*Nyloc+j]=x_left[j]; 
//                }
//                else {
//                    stemp1[i*Nyloc+j]=s[(i+1)*Nyloc+j];
//                    vtemp1[i*Nyloc+j]=v[(i+1)*Nyloc+j];
//                }
//                if (j==0) {
//                    stemp2[i*Nyloc+j]=y_top[i];
//                    vtemp2[i*Nyloc+j]=x_top[i];
//                }
//                else {
//                    stemp2[i*Nyloc+j]=s[i*Nyloc+j-1];
//                    vtemp2[i*Nyloc+j]=v[i*Nyloc+j-1];
//                }
//                if (j==Nyloc-1){
//                    stemp3[i*Nyloc+j]=y_bot[i];
//                    vtemp3[i*Nyloc+j]=x_bot[i];
//                }
//                else {
//                    stemp3[i*Nyloc+j]=s[i*Nyloc+j+1];
//                    vtemp3[i*Nyloc+j]=v[i*Nyloc+j+1]; //After checking all the conditions, v is finally computed 
//                }
//            }
//    }
//}
...
//  int size1=imax-i; 
//  int size2=jmax-j; 
  double* stemp=new double[Nxloc*Nyloc]; //Initialise temp storage variables 
  double* stemp1=new double[Nxloc*Nyloc]; 
  double* stemp2=new double[Nxloc*Nyloc]; 
  double* stemp3=new double[Nxloc*Nyloc];

  double* vtemp=new double[Nxloc*Nyloc]; 
  double* vtemp1=new double[Nxloc*Nyloc]; 
  double* vtemp2=new double[Nxloc*Nyloc];  
  double* vtemp3=new double[Nxloc*Nyloc];  
...
//            cout << "Time step is: " << t << endl << endl; 
             ...
             LidDrivenCavityExp::UpdateTemps(ind);
